	...
	if (buttonIndex == 0)
	{
        switch (apertado) {
            case NO:
                ...
                [self queryTrackCode];
                [self startLocate];
                [self sendSMSWithTrackCode:trackCode];
...
- (void)mailComposeController:(MFMailComposeViewController*)controller
    ...
    NSLog(@"entrou no email cpmposer");
    switch (result)
    {
...
- (void)getBatteryLevel{
    ...
    NSLog(@"nivel bateria %0.2f", batteryLevel);
